...
function adminselect(&$options)
{
		...
		else
		{
			...
			switch($var['type'])
			{
				...
				case 'player':
					...
					// Added hidden value & altered filter field to extend/shrink input --// Timo 01/05/07
					echo '  <input type="hidden" name="list" value="true">';
					echo 'Filter: <input class="searchform" type="text" name="playerfilter" value="'. (empty($_REQUEST['playerfilter']) ? '' : $_REQUEST['playerfilter']) .'" size="'.($options['showlist']==true ? "6" : "24").'">';
					echo '  <input class="searchform" type="Submit" name="noop" value="apply" tabindex="1">';
					break;
	...
	echo '<tr><td>';
	if ($step != 1) echo '<input class="searchformb" type="submit" name="back" value="&lt;&lt; Back" tabindex="3">';
	echo '</td>';
	$caption = ($step == $maxsteps) ? 'Finish' : 'Next &gt;&gt;';
	echo '<td align="right"><input class="searchformb" type="submit" name="submit" value="'.$caption.'"'.($options['showlist']==true ? "" : " DISABLED=\"disabled\"").' tabindex="2"></td></tr>';
