...
//ini_set('memory_limit','100M');
	if ($playerbanned) return;
		if (strpos($gamename, 'Assault') !== false)
		{
			...
			WHERE p.pid = $pid and p.gid = $gid and p.matchid <= $matchid";
		...
		// Update the rank
		$sql = "UPDATE uts_rank SET time = '$rank_gametime', rank = '$rank_nrank', prevrank = '$rank_crank', matches = '$rank_matches' WHERE id = $rank_id;";
