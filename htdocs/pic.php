...
function image_create($filename, &$load, $country) {
	...
	$flag_file = "images/flags/".$country.".png";
	$flag_pic = ImageCreateFromPNG($flag_file);
	$flagW = ImageSX($flag_pic);
	$flagH = ImageSY($flag_pic);
	imagecopy($in, $flag_pic, 5, 11, 0, 0, $flagW, $flagH);

	imagedestroy($im);
	imagedestroy($flag_pic);
...
function get_values($date_from, $date_to, $pid, $gid, $prefix, &$searchrepl) {
	...
	$sql = "	SELECT	m.time AS gamedate,
							...
							AVG(p.eff) AS eff,";
// 	$sql .= "
// 							SUM(kills + deaths + suicides + teamkills ) AS sumeff,";
	$sql .= "
...
$searchrepl = array(	'%GID%'				=>	$gid,
							...
							'%PLAYERCOUNTRY%'	=> strtolower($playercountry),
						);
...
$im = image_create('images/templates/'. $load['template'], $load, $playercountry);
